...
// Asegúrate de que esta interfaz coincida exactamente con la que usa OpportunityForm
interface FormData {
  client_id: number;
  deal_value: number; // Cambiado a number para coincidir con OpportunityForm
  stage: string;
  notes: string;
...
export default function ModalGeneral({ openModal, setOpenModal, id }: ModalGeneralProps) {
  ...
  const [formData, setFormData] = useState<FormData>({
    client_id: 0,
    deal_value: 0, // Inicializado como number
    stage: '',
    notes: '',
    deal_closing_date: '',
  ...
  // Crear una función de wrapper para setFormData que cumpla con la firma esperada por OpportunityForm
