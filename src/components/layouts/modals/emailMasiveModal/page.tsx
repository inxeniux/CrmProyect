...
export default function MassEmailModal({isOpen,onClose}:MassEmailModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("Enviando datos:", payload); // 👀 Verifica que los datos sean correctos
  ...
  const handleStagesFunnel = async (name:number) => {
    try {
      const response = await fetch(`/api/search/stages/${name}`);
  ...
  return (
    <React.Fragment>
      <AiEmailModal setMessage={setMessage} setSubject={setSubject} onClose={setOpenModalEmailAi} isOpen={openModalEmailAi}/>
    <div  className={`fixed inset-0 bg-black bg-opacity-50 z-10 flex justify-center items-end transition-opacity duration-300 ${
      isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
    }`} onClick={()=>onClose(false)}>
    <div        onClick={(e) => e.stopPropagation()}  className={`w-1/2 fixed  right-5 z-50  bottom-5 mx-auto p-6 bg-white shadow-lg transform transition-transform duration-300 ${isOpen ? 'translate-y-0' : 'translate-y-full'} rounded-xl`}>
  ...
  {status.map((item) => (
    <option key={item.stage_id} value={item.stage_id}>
      {item.name}
        ...
        <button onClick={()=>setOpenModalEmailAi(true)} className="bg-black ml-2 text-white rounded-full p-3">
        <BsStars/>
        </button>
        </div>
      </form>
